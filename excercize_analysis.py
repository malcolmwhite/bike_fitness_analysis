...
class rideData:
	...
	def get_param2(self):
		...
		if self.has_good_data:
			...
			scale_hr_param = 150			
			param2 = scale_hr_param * (1 / self.get_hr_at(token_power))
		return param2		
...
class analysis_driver:
	...
	def parse_sys_in(self):
		...
		usage_string += "\t-d specifies directory to look for workout files.\n"
		usage_string += "\t-p specifies decimal precision to use for power data.\n"
		usage_string += "\t-a turns on all secondary plot generation.\n"
